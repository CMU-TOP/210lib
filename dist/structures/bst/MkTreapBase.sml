...
struct
  ...
  fun join (t1, t2) =
    let
      ...
      fun last Leaf = NONE
        | last (Node {data={key, ...}, right, ...}) =
           case right of Leaf => SOME key | _ => last right
      fun first Leaf = NONE
        | first (Node {data={key, ...}, left, ...}) =
           case left of Leaf => SOME key | _ => first left
    in
      case (last t1, first t2) of
        (SOME lLast, SOME rFirst) =>
          if Key.compare (lLast, rFirst) <> LESS then
          raise Order else join' (t1, t2)
      | _ => join' (t1, t2)
    end
